...
const SideBarItem: React.FC<SidebarItemProps> = ({
    ...
    return (
        <Link
            ...
            className={`flex items-center text-black justify-start w-14 group-hover:w-56 ${className}`}
            onClick={(e) => {
                if (onClick) {
        ...
        >
            <div
                className={`flex ${bgColor} flex-row h-11 w-full items-center transition-transform duration-300 py-2 px-2 ${hoverClass}`}
            >
                {children}
            </div>
        </Link>
